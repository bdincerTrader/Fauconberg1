# Editor
# switchBox51   BDincer   5/24/2023 7:57:02 PM   QA-2.0.158
...
class Gr8Script47490160aefa466dafc3a3ed7e518f20(Strategy):
    __script_name__ = 'switchBox51'
    
    tickerItr =['AMC','ARR','BTG','KGC','LUMN','NAT','NOK','NU','SPY'];tkrAlpha=[];
    ...
    allocationModel=10;testRTE=1.0057;riskMax=1500;plMaxLoss=-riskMax*0.20;plUnrlRTE=riskMax*0.001;upperBand=riskMax*.01;lowerBand=-riskMax*.01;bandMult=riskMax*.01;intervalRate=[0];
    

    @classmethod
    def is_symbol_qualified(cls, symbol, md, service, account):
        return symbol in Gr8Scriptd5776c7ea1c14d62828069ab9cdeb56e.tickerItr
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(event.timer_id=="wrt_file" and self.symbol!='SPY'): self.rsk_wrt(account, service);pass;
    ...
    def _hedge_positions(self, ticker, long_capital):
        for unit in self.__class__.tkrAlpha:
            if(unit[0]==ticker):
                self.__class__.HedgeLong+=(long_capital*unit[1]/4)
        ...
        if(len(bar20m.high)>15 and len(bar20m.low)>15 and len(bar20m.spread)>15 and len(bar20m.askvol)>15 and len(bar20m.bidvol)>15 and len(bar20m.close)>15 and len(bar20m.vwap)>15 and len(bar20m.open)>15 and len(bar20m.volume)>15):
            ...
            while(indication>0):
                ...
                if(bar20m.low[-1]>bar20m.high[complexB]):
                    indication+=1
                    pass
                if(bar20m.low[-1]>bar20m.bvwap[complexB]):
                    indication+=1
                    pass
                ...
                if(basePx>bar20m.high[complexB]):
                    indication+=1
                    pass
                if(basePx>bar20m.bvwap[complexB]):
                    indication+=1
                    pass
