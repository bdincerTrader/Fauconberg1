# Editor
# Fauconberg   Baris Dincer   8/26/2023 9:06:32 PM   PROD
...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    bpRisk=5; orderType=0; cycl=6; lclTimeFreq=3; mtmAccountPL=1000; mtmSymbolPL=500; demandBid=0; supplyAsk=0;maxItem=10;
    BENCHMARK_A="QQQ";BENCHMARK_B="SPY"; maxCapUnit=5;
    tickerItrAaRon=['LRCX','COST','SEDG','MCK','ADBE','LPLA','BLK','ASML','PH','URI','ADSK','WDAY','TEAM','ISRG','GNRC','ACN','NXPI', 'EQIX', 'SPY', 'QQQ'];
    FILE_BBS='SHORT_BOOK1.txt'; idxB=0;

    @classmethod
    def is_symbol_qualified(cls, symbol, md, service, account):
        return md.stat.prev_close>20.00 and md.stat.atr>5 and md.stat.avol>175000;
        # return symbol in Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5.tickerItrAaRon;
    ...
    def __init__(self, **params):
        if('riskAllocation' in params):
            ...
            self.__class__.FILE_BBS=params['FILE_BBS'];
            
            print('[riskAllocation]', params['riskAllocation'], '[maxBP]', self.__class__.maxCapUnit,\
            '[maxSymbolTrade]', self.__class__.maxItem, '[timeFreq]', params['timeFreq'], '[cycl]', params['cycl'], '\n',\
            '[orderType]', params['orderType'], '[benchmarkA]', params['benchmarkA'], '[benchmarkB]', params['benchmarkB']);
            pass;
    ...
    def on_start(self, md, order, service, account):
        ...
        self.fileSystemB = service.time_to_string(service.system_time, '%Y-%m-%d')+self.__class__.FILE_BBS;
        self.fileSystemLive = "FauconFileB.txt"
        self.rskB=['SPY','QQQ'];
        ...
        if( (md.stat.prev_close * md.stat.avol) < 100000001):
            self.rskB.append(self.symbol);
            return;
        else:
            service.write_file("000_SCOPE_TKR_FAUCONBERG.txt",'{},{},{},{},{}'.format(self.symbol, md.stat.prev_close, md.stat.avol, self.quvantity, self.riskUnit), mode = 'append', path = {'FORWARD': '/mnt/userdata/bdincer/', 'PRODUCTION': '/mnt/userdata/bdincer/' });
            pass;
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        # hardcoded mtmAccountPL && mtmSymbolPL which is called on any/all timers.
        # System now first observes those two parameters before moving on to do any other transaction.
        # Closes the position using a market order if any of the two parameters are beached without considering a decay of the position.
        
        # ADDITIONAL RISK CHECKS ON THE FLY
        temp_file_risk = service.read_file(self.fileSystemB, path = {'FORWARD': '/mnt/userdata/bdincer/','PRODUCTION': '/mnt/userdata/bdincer/'})
        reader_temp_file_risk = csv.DictReader(io.StringIO(temp_file_risk))
        
        i=0;
        for row in reader_temp_file_risk:
            i+=1;
            if(i>self.__class__.idxB):
                self.__class__.idxB+=1;
                service.write_file(self.fileSystemLive,'{},{}'.format(i, row['CODE']), mode = 'append', path = {'FORWARD': '/mnt/userdata/bdincer/', 'PRODUCTION': '/mnt/userdata/bdincer/' });
                if(row['CODE']=='666'):
                    for tickers in self.rskB:
                        if(row['TICKER'] == tickers):
                            break;
                        else:
                            sor_unitB1 = order.algo_sell(row['TICKER'], algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=int(row['CODE_QTY']), user_key=666.555, allow_multiple_pending=40);
                            bat_unitB1 = order.algo_sell(row['TICKER'], algorithm='398a38ee-b614-4678-90a6-dbf40d2a54b9', intent='increase', order_quantity=int(row['CODE_QTY']), user_key=666.555, allow_multiple_pending=40);
                            ndq_unitB1 = order.algo_sell(row['TICKER'], algorithm='5fc61945-3498-47da-abf6-b5dabdc9f4ac', intent='increase', order_quantity=int(row['CODE_QTY']), user_key=666.555, allow_multiple_pending=40);
                            arc_unitB1 = order.algo_sell(row['TICKER'], algorithm='8fdee8fe-b772-46bd-b411-5544f7a0d917', intent='increase', order_quantity=int(row['CODE_QTY']), user_key=666.555, allow_multiple_pending=40);
                            break;
                        pass;
                    pass;
                elif(row['CODE']=='888.555'):
                    self.rskB.append(row['TICKER']);
                    if(account[row['TICKER']].position.capital_short!=0):
                        cvr_all = order.algo_buy(row['TICKER'], algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='decrease', order_quantity=abs(account[self.symbol].position.shares), user_key=888.555, allow_multiple_pending=40);  
                        pass;
                    pass;
                elif(row['CODE']=='1407.151'):
                    self.rskB.append(row['TICKER']);
                    if(account[row['TICKER']].position.capital_short!=0):
                        close_medium = order.algo_buy(row['TICKER'], algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='decrease', order_quantity=min(abs(account[self.symbol].position.shares), abs(int(row['CODE_QTY']))), user_key=1407.151, allow_multiple_pending=40);  
                        pass;
                    pass;
                elif(row['CODE']=='1407.555'):
                    if(account[row['TICKER']].position.capital_short!=0):
                        close_small = order.algo_buy(row['TICKER'], algorithm='1f9b553d-30bf-40a3-8868-ebe70b5079b2', intent='decrease', order_quantity=min(abs(account[self.symbol].position.shares), abs(int(row['CODE_QTY']))), user_key=1407.555, allow_multiple_pending=40);  
                        pass;
                    pass;
                elif(row['CODE']=='1407.666'):
                    if(account[row['TICKER']].position.capital_short!=0):
                        sor_unitB1 = order.algo_sell(row['TICKER'], algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=abs(int(row['CODE_QTY'])), user_key=666.555, allow_multiple_pending=40);
                        # demit_05 = order.algo_buy(row['TICKER'], algorithm='1f9b553d-30bf-40a3-8868-ebe70b5079b2', intent='increas', order_quantity=min(abs(account[self.symbol].position.shares), int(row['CODE_QTY'])), user_key=1407.666, allow_multiple_pending=40)
                        pass;
                    pass;
                pass;
            pass;
