...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    dntItems=[];
    # tickerItrB = ['GS','MPWR','TMO','ELV','DDS','ORLY','MSCI','EQIX','MSTR','MELI','BKNG','PH','INSP','PANW','PLPC','FIVE','AVGO','URI','FNGU','SPOT','TFX','WING','GWW','LLY','MSCI','URI','FSLR','SWAV','GUSH','REGN','PANW','FIVE','FICO','ZBRA','ASML','SMCI','TMV','LRCX','ACLS','FCNCA'];
    ...
    def is_symbol_qualified(cls, symbol, md, service, account):
        return symbol in Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5.tickerItrH;
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(event.timer_id=="Review"):
            self.itr+=1;
            self.RBCT.append(0);
            self.RBCT_PRIME.append([0,0,0,0]);
            self.chkHop.append([0,0,0]);
            self.midPrice=0.00; self.pxChg=0.00;
            ...
            #print(self.symbol, service.time_to_string(service.system_time), self.ID_RB1)
            ...
            if(len(self.chkHop)>3):
                if(self.RBCT[-1]>self.RBCT[-2] and self.RBCT[-2]==0 and self.chkHop[-1][1]>0):
                    self.RBCT_PRIME[-1][0]=self.RBCT_PRIME[-2]+self.chkHop[-1][2];
                    pass;
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.RBCT_PRIME[-1][3]==1):
            service.write_file(self.filetestB,'{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}'.format(self.itr, self.symbol, service.time_to_string(service.system_time), service.time_to_string(service.system_time),md.L1.bid,md.L1.ask,md.L1.last,self.midPrice, self.chkTrajectoryQ[-1], self.chkTrajectoryS[-1], self.trajectoryBook[-1], self.trajectoryBid[-1], self.trajectoryAsk[-1], self.trajectoryLast[-1], self.trajectorySpread[-1], self.pxTrajectory[-1], self.allocated[-1], 0, self.demand[-1][1], self.demand[-1][2], self.demand[-1][3], self.demand[-1][4], self.demand[-1][6],  self.supply[-1][6], self.supply[-1][4],  self.supply[-1][3], self.supply[-1][2], self.supply[-1][2], (self.marketDepth[0][0]+self.marketDepth[1][0]+self.marketDepth[2][0]),(self.marketDepth[0][1]+self.marketDepth[1][1]+self.marketDepth[2][1]), (self.marketDepth[0][2]+self.marketDepth[1][2]+self.marketDepth[2][2])));
            
            print(self.RBCT_PRIME[-1][0], self.RBCT_PRIME[-1][1], self.RBCT_PRIME[-1][2], self.RBCT_PRIME[-1][3]);
            print('pass');
            ...
            if(self.ID_RB1[-1][6]==1):
                ...
                postB = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=2*self.postUnit, price=self.demand[-1][self.demandGate], user_key=212, allow_multiple_pending=40);
            ...
            if(self.ID_RB1[-1][8]==1):
                ...
                postC = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=3*self.postUnit, price=self.demand[-1][self.demandGate], user_key=212, allow_multiple_pending=40);
            ...
            if(self.ID_RB1[-1][10]==1):
                self.marketDepth[2][0]+=1;
                #self.orderBook.append([self.itr, self.demand[-1][2], self.demand[-1][3], self.demand[-1][4], self.demand[-1][5], self.demand[-1][6], 1]);
                postD = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=4*self.postUnit, price=self.demand[-2][self.demandGate], user_key=212, allow_multiple_pending=40);
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.RBCT_PRIME[-1][3]==0):
            print('demit transaction');


        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1):
    ...
    def on_fill(self, event, md, order, service, account):
        ...
        if(event.intent in ["increase", "init"] and service.instruction_id == event.instruction_id):
            if(event.user_tag==111):
                ...
                if(event.shares==1):
                    self.marketDepth[0][2]+=1;
                    pass;
                elif(event.shares==2):
                    self.marketDepth[1][2]+=1;
                    pass;
                elif(event.shares==3):
                    self.marketDepth[2][2]+=1;
                    pass;
