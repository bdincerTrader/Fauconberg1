...
# Voodoo=8b976c72-20cd-49db-9121-db2e5ace3385 Buy Limit GS SOR at Script Price
 #'398a38ee-b614-4678-90a6-dbf40d2a54b9'  #Sell Market BATS 
...
class Gr8Scriptcffda23257ae4d54a108e8990036b304(Strategy):
    ...
    def __init__(self, **params):
        if('riskAllocation' in params):
            self.__class__.bpRisk=params['riskAllocation'];  self.__class__.bpMaxLoss=-self.__class__.bpRisk*0.20;
            self.__class__.plUnrlRTE=self.__class__.bpRisk*params['plRiskBp'];  self.__class__.lclTimeFreq=params['timeFreq'];
            self.__class__.plUpperBand=self.__class__.bpRisk*.01; self.__class__.plLowerBand=-self.__class__.bpRisk*.01;
            self.__class__.plBandMult=self.__class__.bpRisk*.01; self.__class__.cycl=params['cycl']; self.__class__.askRTE=params['askRTE'];
            self.__class__.plSpr=params['plRoute']; self.__class__.pxChk=params['mkSpread']; self.__class__.orderType=params['orderType'];
            self.__class__.BENCHMARK_A=params['benchmarkA']; self.__class__.BENCHMARK_B=params['benchmarkB'];
    ...
    def on_start(self, md, order, service, account):
        ...
        service.add_time_trigger(md.market_open_time + service.time_interval(minutes=5), repeat_interval=service.time_interval(seconds=int(self.__class__.lclTimeFreq+20)), timer_id="Review")
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(account[self.symbol].position.shares>0):
            if(account[self.symbol].position.shares>abs(account[self.symbol].pending.shares_short)):
                print(account[self.symbol].position.mtm_price, self.__class__.askRTE, self.trajectorySpread[-1], md.L1.bid, md.L1.ask, md.L1.last);  
                chkSpread=max(self.__class__.askRTE, self.trajectorySpread[-1]);
                exitPrice=account[self.symbol].position.mtm_price*self.__class__.askRTE;
                offerInventory = order.algo_sell(self.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='decrease', order_quantity=(account[self.symbol].position.shares+account[self.symbol].pending.shares_short), price=exitPrice, user_key=2121, allow_multiple_pending=40);
