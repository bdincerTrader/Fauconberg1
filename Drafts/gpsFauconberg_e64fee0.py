...
class Gr8Scriptac8077fd131647b7a7ee64fee0e9cbda(Strategy):
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(self.systemStatus==1):
            print('SYSTEM_STATUS_UPDATE', self.symbol, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.ATR_TypeB, self.motionA,  self.motionY, self.ATR_TypeA, self.motionB, self.motionX, self.orderSession, self.bot, self.sold, self.neutral, self.ebft, self.orderBalance)
            self.satisfied=0;
            self.bot=0;
            self.sold=0;
            self.long=0;
        ...
        if(self.key==1 and event.timestamp<self.illiquid_time): 
            ...
            if(self.ebft<0):
                ...
                if(len(self.levelThree)>2):
                    ...
                    print('[-]ebft','ct', 'session',  'tkr', 'position','prior', 'gps', 'clear','bid', 'last', 'ask', 'ebft', 'neutral', 'allocationRate', '[-]','[-L]','[+]','[+L]','long_bal', 'statusShort','statusUpdate', 'gps', 'b', 's', 'B','S','[rate]','[rate-1]','[rate-2]','[rateAVG]','[n]','[n-1]','[n-2]','[n_AVG]');
                    print('down0.0', self.orderBalance, self.orderSession, self.symbol, short_position, currentLocation, self.satisfied, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.ebft, self.neutral, self.allocationRate, (self.ATR_scR+self.mot_scR+self.xxl_scR),self.systemDOWN[-2],(self.ATR_scRp+self.mot_scRp+self.xxl_scRp),self.systemUP[-2], (self.bot-self.long), (self.short+self.long), ((self.short+self.long)-self.orderBalance+self.long-self.satisfied), self.orderBalance, self.bot, self.sold, self.long, self.short,spot1,spot2,spot3,spot4,spot5,spot6,spot7,spot8);
                ...
                longShares=shortShares=longUnit=shortUnit=0;
                longPrice=shortPrice=0;
                longAllocation=shortAllocation=0;
                shortLevel=longLevel=0;
                while(pendingOrderUnit>0):
                    pendingOrderUnit-=1;
                    caseOrder=pendingOrderBook[pendingOrderUnit];
                    if(caseOrder.shares<=-1):
                        shortUnit+=1;
                        shortShares+=caseOrder.shares;
                        shortPrice+=caseOrder.price;
                        shortAllocation=shortPrice*shortShares;
                        shortLevel=shortAllocation/shortShares;
                        print('offers', 'short', self.orderSession, caseOrder.timestamp, caseOrder.price, caseOrder.shares, caseOrder.user_tag, short_position, self.symbol, currentLocation, self.satisfied, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.ebft, self.neutral, self.allocationRate, (self.ATR_scR+self.mot_scR+self.xxl_scR),self.systemDOWN[-2],(self.ATR_scRp+self.mot_scRp+self.xxl_scRp),self.systemUP[-2], (self.bot-self.long), (self.short+self.long), ((self.short+self.long)-self.orderBalance+self.long-self.satisfied), self.orderBalance, self.bot, self.sold, self.long, self.short, spot1,spot2,spot3,spot4);
                        pass;
                    if(caseOrder.shares>=1):
                        longUnit+=1;
                        longShares+=caseOrder.shares;
                        longPrice+=caseOrder.price;
                        longAllocation=longPrice*longShares;
                        longLevel=longAllocation/longShares;
                        print('bids', 'cover', self.orderSession, caseOrder.timestamp, caseOrder.price, caseOrder.shares, caseOrder.user_tag, short_position, self.symbol, currentLocation, self.satisfied, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.ebft, self.neutral, self.allocationRate, (self.ATR_scR+self.mot_scR+self.xxl_scR),self.systemDOWN[-2],(self.ATR_scRp+self.mot_scRp+self.xxl_scRp),self.systemUP[-2], (self.bot-self.long), (self.short+self.long), ((self.short+self.long)-self.orderBalance+self.long-self.satisfied), self.orderBalance, self.bot, self.sold, self.long, self.short, spot1,spot2,spot3,spot4);
                        
                        #print('BUYORDER', caseOrder);
                        pass;
                
                if(len(pendingOrderBook)!=0):
                    print('ORDERBOOK', self.orderSession, account[self.symbol].position.shares, self.long, self.short, self.long+self.short, self.orderBalance, self.satisfied, longUnit, longShares, longAllocation, longLevel, shortUnit, shortShares, shortAllocation, shortLevel,(self.ATR_scR+self.mot_scR+self.xxl_scR),self.systemDOWN[-2],(self.ATR_scRp+self.mot_scRp+self.xxl_scRp),self.systemUP[-2],0, 0, 0, 0, self.bot, self.sold, spot1,spot2,spot3,spot4,spot5,spot6,spot7,spot8);
                    pass;
                    
                #if(self.orderBalance<account[self.symbol].position.shares):
                #    print('\n\nrebalance');
                #    if(account[self.symbol].position.shares>0):
                #        reBalance= self.orderBalance-account[self.symbol].position.shares;
                #        return order.algo_sell(self.symbol, algorithm='5fc61945-3498-47da-abf6-b5dabdc9f4ac', intent='reverse', order_quantity=reBalance, user_key=1011, allow_multiple_pending=True);
                        
                lvlDOWN=abs(self.ATR_scR+self.mot_scR+self.xxl_scR);
                lvlUP=self.ATR_scRp+self.mot_scRp+self.xxl_scRp;
                statusUpdate=0;         
                if(lvlDOWN>=2 and len(self.systemDOWN)>1):
                
                    long_bal=self.bot-self.long;
                    statusShort=self.short+self.long;
                    statusUpdate=abs(self.orderBalance-short_position)-self.satisfied;
                    ...
                    if(statusShort>self.orderBalance):
                        ...
                        if(account[self.symbol].position.shares>0):
                            print('\n\nreversal');
                            orderSize=max(short_position, statusUpdate);
                            order.algo_sell(self.symbol, algorithm='5fc61945-3498-47da-abf6-b5dabdc9f4ac', intent='reverse', order_quantity=orderSize, user_key=888, allow_multiple_pending=True);
                            pxGeneric=min(md.L1.bid, md.L1.last)-(self.p5);
                            order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=abs(int(statusUpdate/2)+1), price=pxGeneric, user_key=999, allow_multiple_pending=True);
                            statusUpdate=0;
                            pxGeneric=min(md.L1.bid, md.L1.last)-(self.p1);
                            order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=abs(int(statusUpdate/2)+1), price=pxGeneric, user_key=999, allow_multiple_pending=True);
                            statusUpdate=0;
                            pass;
                        elif(statusUpdate>0):
                            order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=abs(int(statusUpdate/2)+1), user_key=1011, allow_multiple_pending=True);
                            statusUpdate=0;
                            pxGeneric=min(md.L1.bid, md.L1.last)-(self.p5);
                            order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=self.neutral+1, price=pxGeneric, user_key=999, allow_multiple_pending=True);
                            pass;
                        else:
                            statusUpdate=0;
                            pxGeneric=min(md.L1.bid, md.L1.last)-(self.p5);
                            order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=-self.ebft+self.neutral, price=pxGeneric, user_key=999, allow_multiple_pending=True);
                            pxGenericB=min(md.L1.bid, md.L1.last)-(self.p1);
                            order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=(-self.ebft+self.neutral)*2, price=pxGenericB, user_key=999, allow_multiple_pending=True);
                            statusUpdate=0;
                            pass;
    ...
    def on_fill(self, event, md, order, service, account):
        ...
        if(event.user_tag==999):
            ...
            self.satisfied+=abs(event.shares);
            pass;
        return;
