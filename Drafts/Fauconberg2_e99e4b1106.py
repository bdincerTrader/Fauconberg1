...
class Gr8Script2732dd2ce3974c439ecdd5e99e4b1106(Strategy):
    __script_name__ = 'Fauconberg2'
    ...
    def is_symbol_qualified(cls, symbol, md, service, account):
        return symbol in Gr8Script2732dd2ce3974c439ecdd5e99e4b1106.tickerItrB;
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(self.key==1):
            ...
            print('ebft',  service.time_to_string(service.system_time), self.symbol, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.chk_SPY, self.chk_QQQ,  currentLocation, self.__class__.spyCHG, self.__class__.qqqCHG, currentRate, 1, -1,  self.neutralized,   self.neutral, self.ebft, self.orderBalance);
            ...
            if(self.ebft<0):
                ...
                if(lvlDOWN>=5):
                    if(self.orderBalance<0):
                        order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=abs(self.orderBalance), user_key=lvlDOWN);
                        pass;
                    pass;
                print('ebft exchg',  service.time_to_string(service.system_time), self.symbol, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.chk_SPY, self.chk_QQQ,  currentLocation, self.__class__.spyCHG, self.__class__.qqqCHG, currentRate, self.ATR_scRp+self.mot_scRp+self.xxl_scRp, self.ATR_scR+self.mot_scR+self.xxl_scR,  'down', self.ebft, self.neutral, self.orderBalance);
            ...
            if(self.ebft>0):
                ...
                if(lvlUP>=5):
                    if(self.orderBalance>0):
                        order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='increase', order_quantity=abs(self.orderBalance), user_key=lvlUP);
                        pass;
                    pass;
                    
                print('ebft exchg',  service.time_to_string(service.system_time), self.symbol, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.chk_SPY, self.chk_QQQ,  currentLocation, self.__class__.spyCHG, self.__class__.qqqCHG, currentRate, self.ATR_scR+self.mot_scR+self.xxl_scR, self.ATR_scRp+self.mot_scRp+self.xxl_scRp,  'up', self.ebft, self.neutral, self.orderBalance);
