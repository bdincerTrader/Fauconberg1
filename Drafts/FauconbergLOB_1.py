...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    dntItems=[];
    tickerItrA=['INSP','PANW','PLPC','FIVE','FICO','ZBRA','ASML','SMCI','TMV','URI','FNGU','SPOT','MSCI','EQIX','MSTR','MELI','PH','ACN','ADBE','ADSK','ALB','ALGN','ALNY','AMD','AMP','AMT','ANET','ANSS','AON','APD','ARGX','ASML','AVGO','AZO','BA','BDX','BIDU','BIIB','BILL','BKNG','BLK','MPWR','BURL','CAR','CAT','CDNS','CELH','CHTR','CI','CMG','CMI','COIN','COST','CRWD','CSL','CTAS','CVNA'];
    ...
    def is_symbol_qualified(cls, symbol, md, service, account):
        #return symbol in Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5.tickerItrB;
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(event.timer_id=="Review"):
            ...
            if(len(self.ID_R1)>1):
                if(self.ID_R1[-2]==0 and self.ID_R1[-1]!=0):
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.ID_R3[-1][9]==0):
            print('demit transaction', self.symbol, service.time_to_string(service.system_time), self.midPrice);
            pass;
