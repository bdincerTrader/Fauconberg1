# Editor
# bftCopy1   Baris Dincer   7/23/2023 6:09:07 PM   PROD
...
class Gr8Script7440a7b776ad48b280810fd86a9748af(Strategy):
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(self.key==1):
            ...
            if(self.ebft<0):
                ...
                shortBal=0
                if(self.netShortUnit>0):
                    shortBal=self.netShortPx/self.netShortUnit;
                    pass;
                    
                print('book status', self.sold, self.short, self.long, self.bot);
                ...
                if(len(self.systemDOWN)>1):
                    print(self.systemDOWN[-2], lvlDOWN)
                    long_bal=self.bot-self.long;
                    statusShort=self.short+self.long;
                    statusUpdate=0;
                    if(statusShort*1.1>self.orderBalance):
                        print('statusUpdate', statusUpdate, long_bal, self.orderBalance,  self.sold, self.short, self.long, self.bot);
                        statusUpdate=statusShort-self.orderBalance;
                        return order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=statusUpdate, user_key=1011);

                    if(shortBal>0 and long_bal*1.25>self.bot):
                        print('riskAdjust', statusUpdate, long_bal, self.orderBalance,  self.sold, self.short, self.long, self.bot);
                        order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='decrease', order_quantity=long_bal, user_key=2021);
                        
                    print('systemDOWN', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                    if(lvlDOWN>=3 and lvlDOWN>self.systemDOWN[-2] and statusUpdate>0):
                        upDown = self.short+self.sold-self.bot;
                        print('lclDOwn', upDown, self.sold, self.short, self.bot);
                        print('system Lower 1011', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                        return order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=statusUpdate, user_key=1011);
                        pass;
                    elif(lvlUP>0 and lvlUP>self.systemUP[-2] and abs(account[self.symbol].position.shares)!=0):
                        if(shortBal>0 and long_bal>0):
                            return order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='decrease', order_quantity=self.neutral+abs(self.ebft), user_key=2021);
                            pass;
                        pass;
                    elif(lvlUP>0 and lvlUP<self.systemUP[-2] and statusUpdate>0):
                        print('system Lower 2019', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                        return order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=statusUpdate, user_key=2019);    
                    elif(shortBal>0 and long_bal>0 and self.neutral>0):
                        pxTwo=md.L1.bid-self.p1;
                        print('cover2', statusUpdate, pxTwo)
                        return order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=self.neutral, price=pxTwo, user_key=2011, allow_multiple_pending=True);
                    elif(lvlDOWN>=3 and lvlDOWN==self.systemDOWN[-2]):
                        print('check system bal', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                        if(statusUpdate>0):
                            print('update system bal', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                            return order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=int(statusUpdate), user_key=1011);
                        pass;
                    else:                    
                        if(shortBal>0 and long_bal>0 and self.neutral>0):
                            pxFour=md.L1.bid-self.p1;
                            print('cover4', statusUpdate, pxFour);
                            return order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=self.neutral, price=pxFour, user_key=2011, allow_multiple_pending=True);
                            pass;
                        pass;
                        
                
                #if(lvlDOWN>=3):
                #    order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=lvlDOWN, user_key=1013);
                #    pass;
    ...
    def on_fill(self, event, md, order, service, account):
        if(event.user_tag==2021):
            print('\n\n\n ###CLEAR');
        if(event.user_tag==2019):
            print('\n\n\n ALLOCATE'); 
        if(event.shares<0):
