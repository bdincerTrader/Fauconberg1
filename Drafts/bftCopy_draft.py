...
class Gr8Script7440a7b776ad48b280810fd86a9748af(Strategy):
        ...
        if(self.key==1):
            ...
            if(self.ebft<0):
                ...
                if(len(self.systemDOWN)>1):
                    ...
                    if(lvlDOWN>=3 and lvlDOWN>self.systemDOWN[-2]):
                        upDown = self.sold-self.short;
                        print('lclDOwn', upDown, self.sold, self.short);
                        order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=abs(upDown), user_key=1011);
                        pass;
                    elif(lvlDOWN>=3 and lvlDOWN==self.systemDOWN[-2]):
                        upDown = self.short-self.sold;
                        order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=abs(upDown), user_key=1011);
                        if(lvlUP>0 and lvlUP>self.systemUP[-2]):
                            print('lvlUP', self.bot, self.long)
                            downUp = int(self.bot-self.long);
                            order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='decrease', order_quantity=downUp, user_key=2011);
                            pass;
                            
                
                if(lvlDOWN>=3):
                    order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='increase', order_quantity=lvlDOWN, user_key=1011);
                    pass;
    ...
    def on_fill(self, event, md, order, service, account):
        if(event.user_tag==1011):
            self.short+=abs(event.shares);
        if(event.user_tag==2011):
            self.long+=abs(event.shares);
        self.neutralized-=1;
        pass;
