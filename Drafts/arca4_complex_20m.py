...
class Gr8Scriptf6fd3016e01f42ca9b743a1888644643(Strategy):
    __script_name__ = 'arca4_complex'
    
    allocationModel=10;testRTE=1.0057;riskMax=1500;lvlTwo=0.9996;maxLoss=1.02;clockOne=3;clockTwo=31;
    ...
    def on_start(self, md, order, service, account):                      
        ...
        self.decreaseRisk = [];
        timerOne=self.__class__.clockOne;
        timerTwo=self.__class__.clockTwo;
        print("TKR:{}\tMODEL: {}\t MKT SPR {}.{} \t TIMERS: {}|{}\tTRADE DATE: {}".format(self.symbol, self.riskModel, self.mktSpread, self.mktAdjust, timerOne, timerTwo, service.time_to_string(service.system_time)))
        
        service.add_time_trigger(md.market_open_time + service.time_interval(minutes=5), repeat_interval=service.time_interval(seconds=timerOne), timer_id="Allocate")
        service.add_time_trigger(md.market_open_time + service.time_interval(minutes=5), repeat_interval=service.time_interval(seconds=timerTwo), timer_id="Release")
                ...
                for allocation in self.decreaseRisk:
                    if(account[self.symbol].position.capital_long>0):
                        if((allocation[2]*self.lossTolerance)<md.L1.bid):
                            negative = order.algo_sell(event.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='decrease', order_quantity= allocation[1], price=md.L1.bid*self.mktAdjust, user_key=112, allow_multiple_pending=40);
                            pass;
                        elif((allocation[2]*self.mktAdjust)>md.L1.bid):
                            positive = order.algo_sell(event.symbol, algorithm='97a91492-1033-4a96-a546-9bff6df73b08', intent='decrease', order_quantity=allocation[1], price=md.L1.bid, user_key=112, allow_multiple_pending=40);
                            pass;
                        elif((allocation[2]*self.mktAdjust)>md.L1.last):
                            positive = order.algo_sell(event.symbol, algorithm='97a91492-1033-4a96-a546-9bff6df73b08', intent='decrease', order_quantity=allocation[1], price=md.L1.last, user_key=112, allow_multiple_pending=40);
                            pass;
                        else:
                            neutral = order.algo_sell(event.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='decrease', order_quantity= allocation[1], price=md.L1.ask, user_key=112, allow_multiple_pending=40);
                            pass;
                self.decreaseRisk=[];
    ...
    def on_fill(self, event, md, order, service, account):
        if(event.user_tag==111):
            ...
            if(event.intent in ["increase", "init"] and service.instruction_id == event.instruction_id):
                offerInventory = order.algo_sell(event.symbol, algorithm='97a91492-1033-4a96-a546-9bff6df73b08', intent='decrease', order_quantity= event.shares, price=pxOne, user_key=112, allow_multiple_pending=40)
    ...
    def on_cancel(self, event, md, order, service, account):
        if(event.user_tag==112 and event.timestamp<self.illiquid_time):
            self.decreaseRisk.append(event.symbol, event.shares, event.price);
            pass;
            ...
            if(indication>0):
                ...
                if(indication>=self.riskModel):
                    ...
                    if(account[self.symbol].position.shares==0 and len(account[self.symbol].pending.orders)<40):
                        riskUnit = order.algo_buy(self.symbol, algorithm='7a6228ad-dc9d-4377-8b2e-99e8ebb90152', intent='increase', order_quantity=bidQty, price=bidPx, user_key=111, allow_multiple_pending=40)
                        return riskUnit;
                    pass;
                pass;
            pass;
