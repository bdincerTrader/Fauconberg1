...
class Gr8Scriptac8077fd131647b7a7ee64fee0e9cbda(Strategy):
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(self.key==1 and event.timestamp<self.illiquid_time): 
            ...
            if(self.ebft<0):
                ...
                if(len(self.systemDOWN)>1):
                    ...
                    if(statusShort*1.1>self.orderBalance):
                        ...
                        print('statusUpdate', long_bal, statusUpdate, long_bal, self.orderBalance,  self.sold, self.short, self.long, self.bot, pxGeneric);
                        pass;
                    ...
                    if(lvlDOWN>=3 and lvlDOWN>self.systemDOWN[-2] and statusUpdate>0):
                        ...
                        print('system LOWER 1112:\t', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                        ...
                        print('system LOWER 101:\t', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                        ...
                        ## order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=abs(self.ebft), user_key=113, allow_multiple_pending=40);
                        pxLowerBid=min(md.L1.bid, md.L1.last)-(self.p1*1);
                        print('cover 101:\t', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot, pxLowerBid);
                    ...
                    if(lvlDOWN>=3 and lvlDOWN==self.systemDOWN[-2] and statusUpdate>0):
                        print('system LOWER 1111:\t', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot);
                        ...
                        print('cover:\t', long_bal, statusUpdate, self.sold, self.short, self.long, self.bot, pxTwoBid);
            ...
            if(self.orderSession>0 and self.ebft==0):
                ...
                if(self.orderBalance<0 or abs(account[self.symbol].position.shares)>0):
    ...
    def on_fill(self, event, md, order, service, account):
        if(event.shares<0):
            print('SELL\t', event.shares, event.price)
            self.short+=event.shares;
            self.netShortPx+=event.price;
            self.netShortUnit+=1;
            return;
        if(event.shares>0):
            print('BUY\t', event.shares, event.price)
            self.long+=event.shares;
            return;
        if(event.user_key==999):
