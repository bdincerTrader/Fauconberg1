# Editor
# Fauconberg   BDincer   8/2/2023 4:34:00 PM   QA-2.0.203
...
# Fauconberg   BDincer   8/02/2023 5:58:59 AM   QA-3.0.158  -NO BID. ONLY.
# BENCHMARK_A="QQQ"; BENCHMARK_B="SPY"; >>Can be adjusted in params.
...
# Voodoo=5a8a9f9d-22a5-4155-87e7-4ebe8024a97b Sell Limit GS SOR at Script Price
# Voodoo=8b976c72-20cd-49db-9121-db2e5ace3385 Buy Limit GS SOR at Script Price
#'398a38ee-b614-4678-90a6-dbf40d2a54b9'  #PRICE CHECK UPDATE
 
from cloudquant.interfaces import Strategy
import time, datetime, random

class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    __script_name__ = 'a97bb614'
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(event.timer_id=="Review"):
            ...
            if(len(self.allocated)>5):
                if(self.allocated[-1]>self.allocated[-2]>self.allocated[-3] and self.allocator>1):
                    ...
                    self.safetySymbol=640; self.orderECN=0; return 1;
                elif(self.allocated[-1]<self.allocated[-2] and self.allocator>1):
                    self.safetySymbol=640; self.orderECN=0; return 1;
                elif(self.allocated[-1]<self.allocated[-2] and self.allocator<1):
                    ...
                    self.orderECN=-1;
                    return -1;
                elif(self.allocated[-1]<self.allocated[-2] and self.allocated[-2]<self.allocated[-3]):
                    # switch
        ...
        if(self.allocator<0):
            ...
            if(account[self.symbol].position.shares>0):
                closeSwitch = order.algo_sell(self.symbol, algorithm='5a8a9f9d-22a5-4155-87e7-4ebe8024a97b', intent='exit', user_key=97200, allow_multiple_pending=40);
                return closeSwitch;
            pass;
        
        if(self.safetySymbol==750 and account[self.symbol].position.shares>0):
            if(account[self.symbol].position.shares>0):
                closeSwitch = order.algo_sell(self.symbol, algorithm='5a8a9f9d-22a5-4155-87e7-4ebe8024a97b', intent='exit', user_key=97200, allow_multiple_pending=40);
                return closeSwitch;
            pass;    
        ...
        if(self.allocator<0 or self.safetySymbol==750):
            ...
            if(ctOrders!=0):
                ...
                png97b = order.algo_sell(self.symbol, algorithm='5a8a9f9d-22a5-4155-87e7-4ebe8024a97b', intent='exit', user_key=97200, allow_multiple_pending=40);
                return -2;
        elif(self.allocator>1 and self.safetySymbol!=750):
            self.safetySymbol=640;
            pass;
        elif(self.allocator<0):
            return 0;
        
        def procPL_3(sprPam):
            chkPxA=0;
            if(sprPam==3): qbA=md.L1.ask-((md.L1.ask-md.L1.bid)/sprPam); reRack=max(md.L1.last, qbA)+self.mkOffer;pass;
            elif(sprPam==4): qbA=md.L1.ask-((md.L1.ask-md.L1.bid)/sprPam); reRack=max(md.L1.last, qbA)+self.mkOffer;pass;
            ...
            self.venue+=1;
            if(self.venue%2==1 and sizeAgg>0 and account[self.symbol].position.shares>0):
                newLevel = order.algo_sell(self.symbol, algorithm='09528d6b-7d4f-4cc2-8611-8bbecb33785c', intent='decrease', order_quantity=sizeAgg, price=reRack, user_key=112, allow_multiple_pending=40);
                pass;
            elif(self.venue%2==0 and sizeAgg>0 and account[self.symbol].position.shares>0):
                newLevel = order.algo_sell(self.symbol,  algorithm='f8328375-d95c-4c10-aa7e-0c7fecf13336', intent='decrease', order_quantity=sizeAgg, price=reRack, user_key=112, allow_multiple_pending=40)
                pass;
            self.procPL21=0;
            self.procPL2=0;
            return 1;
        ...
        def procPL_1():
            def procPL_2():
                ...
                print('TimerB', service.time_to_string(service.system_time), event.timestamp, self.orderTimerB, self.orderTimerA);
                if(event.timestamp+1691209231>self.orderTimerB-21000000):
                    self.orderTimerB=event.timestamp;
                    while(ctPend>0):
            ...
            def procPL_21():
                adjustAsk=account[self.symbol].pending.orders; ctPend=len(adjustAsk);
                print('TimerA', service.time_to_string(service.system_time), event.timestamp, self.orderTimerA, self.orderTimerB);
                if(event.timestamp+1691209231>self.orderTimerA-21000000):
                    self.orderTimerA=event.timestamp;
                    while(ctPend>0):
                        ctPend-=1; pendingUnit=adjustAsk[ctPend];
                        if(pendingUnit.shares<0):
                            discharge = order.cancel(order_id=pendingUnit.order_id);
        ...
        if event.timer_id=="eod_2":
            if(account[self.symbol].position.shares>0):
                closeInventory = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='exit', user_key=113, allow_multiple_pending=40);
                        ...
                        if(len(self.dataBook[0])>1):
                            ...
                            if(dmp>self.cycle and abs(account[self.symbol].position.capital_long)<self.riskAllowance*10 and len(account[self.symbol].pending.orders)<10 and self.quvantity>0):
                                ...
                                if(self.orderECN==1 and abs(account[self.symbol].position.capital_long)<self.riskAllowance*5):
                                    ...
                                    riskUnitOne = order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='increase', order_quantity=min(self.quvantity, int(self.allocator/2))+3, user_key=111, allow_multiple_pending=40);
                                    return riskUnitOne;
                                    ...
                                    lmtOrd = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=min(self.quvantity, int(self.allocator/2))+3, price=mkPx, user_key=111, allow_multiple_pending=40);
                                    return lmtOrd;
                            else: return 0;
                        else: return 0;
                    else: return 0;
    ...
    def on_fill(self, event, md, order, service, account):
        if(event.intent in ["increase", "init"] and service.instruction_id == event.instruction_id):
            if(event.user_tag==111):
                if(len(self.trajectorySpread)>4):
                    mktSprB = sum(self.trajectorySpread[-3:]) / len(self.trajectorySpread[-3:]);
                    mktSprA = max(self.offerGeneric, (mktSprB/2));
                    offerInventory = order.algo_sell(event.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='decrease', order_quantity=event.shares, price=(event.price+mktSprA), user_key=112, allow_multiple_pending=40);
