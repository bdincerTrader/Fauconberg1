# Editor
# bftCopy1   Baris Dincer   7/22/2023 10:40:29 PM   PROD
...
class Gr8Script7440a7b776ad48b280810fd86a9748af(Strategy):
    ...
    def on_start(self, md, order, service, account):
        ...
        self.systemUP=self.systemDOWN=[0]
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        def printBars(_stage, _location, _currentRate, _mxSpr, _mnSpr):                
            print(_stage, service.time_to_string(service.system_time), self.symbol, self.zero, md.L1.bid, md.L1.last, md.L1.ask, self.chk_SPY, self.chk_QQQ,  _location, self.__class__.spyCHG, self.__class__.qqqCHG, _currentRate, self.ATR_TypeA, self.ATR_TypeB, self.motionA, self.motionB, self.motionX, self.motionY, '212', self.priceBook[-1], (self.priceBook[-1]-self.priceBook[-2]), (self.priceBook[-1]-self.priceBook[0]), mxSpr, mnSpr, 212, self.orderSession, self.bot, self.sold, self.neutral, self.ebft, self.orderBalance);
            return
        ...
        if(self.key==1):
            ...
            print(self.systemUP[-1], self.systemUP[-2])
            
            if(self.systemUP[-1]!=self.systemUP[-2]):
                printAction('(+)EXCH', currentLocation, currentRate, self.systemUP[-1], self.systemUP[-2], nAction, '(+)EXCH');
            
            print(self.systemDOWN[-1], self.systemDOWN[-2])
            if(self.systemDOWN[-1]!=self.systemDOWN[-2]):
                printAction('(-)EXCH', currentLocation, currentRate, self.systemUP[-1], self.systemUP[-2], nAction, '(-)EXCH');
