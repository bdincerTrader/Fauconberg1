# FauconbergBOOKTimer409   BDincer   8/13/2023 9:42:30 PM   QA-2.0.205
...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    def __init__(self, **params):
        if('riskAllocation' in params):
            ...
            self.__class__.plUnrlRTE, '\n', '[maxBP]', self.__class__.maxCapUnit, 'timeFreq..', params['timeFreq'], '\n','cycl', params['cycl'], '\n', 'askRTE',\
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        if(event.timer_id=="Review"):
            ...
            if(account[self.symbol].position.shares>0 and self.ID_R4[-1][12]>0):
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.ID_R3[-1][9]==1 and self.postUnit>0):
            # self.offerGenericZero                         ADJ vs. RTE of CHG.
        ...
        if(self.chkHtz>0):
            ...
            if(account[self.symbol].position.shares>0 and self.qCtr>self.qtyZero):
                ...
                print(self.qCtr, self.chkHtz, adjustRiskB);
                
                demitRisk = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=self.qCtr, user_key=116, allow_multiple_pending=40);
                
                if(account[self.symbol].position.mtm_price<md.L1.last):
                    postLMT = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=self.postUnit, price=self.demand[-2][1], user_key=212, allow_multiple_pending=40);
                    pass;                
            else:
                self.chkHtz=0;

        # CHECKS FOR A TERMINAL CONDITION AND DEMITS RISK ACCORDINGLY.
