...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        self.plFlip=account[self.symbol].position.capital_long/100;     
        ...
        if(account[self.symbol].unrealized_pl.entry_pl>self.plFlip):
            self.order_quantity_max+=self.riskAllowance;
            clsUnitUnrl = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=self.postUnit, user_key=1912, allow_multiple_pending=40);
            pass;
        if(account[self.symbol].position.mtm_price<md.L1.bid and account[self.symbol].position.capital_long>0):
            self.order_quantity_max+=self.riskAllowance;
            clsUnitBid = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=self.postUnit, user_key=1912, allow_multiple_pending=40);
            pass;

        self.order_quantity_max=min(abs(account.buying_power-self.limitBP), self.order_quantity_max);
        
        print('limit risk');
        print(self.order_quantity_max, self.limitBP);
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.ID_R3[-1][9]==11 and self.postUnit>0):
