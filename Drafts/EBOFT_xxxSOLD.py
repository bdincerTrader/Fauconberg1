            ...
            if(self.ebft<0):
                ...
                if(lvlDOWN>=2 and len(self.systemDOWN)>1):
                    ...
                    if(statusShort>self.orderBalance):
                        ...
                        if(account[self.symbol].position.shares>0):
                            print('\n\nreversal');
                            order.algo_sell(self.symbol, algorithm='5fc61945-3498-47da-abf6-b5dabdc9f4ac', intent='reverse', order_quantity=statusUpdate, user_key=888, allow_multiple_pending=True);
                            pxGeneric=min(md.L1.bid, md.L1.last)-(self.p5);
                            order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='decrease', order_quantity=int(statusUpdate/2)+1, price=pxGeneric, user_key=999, allow_multiple_pending=True);
                            statusUpdate=0;
