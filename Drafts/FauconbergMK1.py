...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    def on_timer(self, event, md, order, service, account):
        self.limitBP=self.order_quantity_max-account[self.symbol].pending.capital_long-account[self.symbol].pending.capital_long;
        self.plFlip=account[self.symbol].pending.capital_long/100;
        if(account[self.symbol].unrealized_pl.entry_pl>self.plFlip):
            clsUnitRisk = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=self.postUnit, user_key=1912, allow_multiple_pending=40);
            pass;
        if(account[self.symbol].position.mtm_price<md.L1.bid):
            clsUnitRisk = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=self.postUnit, user_key=1912, allow_multiple_pending=40);
            pass;
                        ...
                        if(len(self.dataBook[0])>1):
                            ...
                            if(dmp>self.cycle and self.quvantity>0):
                                ...
                                if(self.orderECN==1):
                                    ...
                                    riskUnitLMT = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=bidQty, price=zeroPx, user_key=111, allow_multiple_pending=40);
                                    self.qCtr=0;
