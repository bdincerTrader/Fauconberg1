...
class Gr8Script17b09ac5f7834bbcba0a0f40e2eb1ec5(Strategy):
    ...
    def __init__(self, **params):
        if('riskAllocation' in params):
            ...
            self.__class__.dntItems.append(params['doNotTrade']);
    ...
    def on_start(self, md, order, service, account):
        ...
        self.filetestC=self.symbol+service.time_to_string(service.system_time, '%Y-%m-%d')+"-oBook.txt";
        ...
        self.offerGenericZero=self.offerGeneric;
        self.sizeAgg=0;
        
        self.limitBP=account.buying_power/4;
        
        print('[self.limitBP]', self.limitBP, '[account.buying_power]', account.buying_power);
    ...
    def on_timer(self, event, md, order, service, account):
        self.order_quantity_max=(account.buying_power/2)-(account[self.symbol].position.capital_long)-(self.riskUnit*5);
        ...
        if(event.timer_id=="Review"):
            ...
            if(account[self.symbol].position.shares>0 and self.ID_R4[-1][13]>0):
                self.SWITCH_BOOK.append([account[self.symbol].position.shares, account[self.symbol].pending.shares_long, account[self.symbol].pending.shares_short, self.hopHZ, 0.00]);
                
                # SWITCHES THE ORDER BOOK SYSTEM FROM 640 TO 555.
                if(self.qtyStat!=555):
                    self.qtyStat=555;
                    adjustAsk=account[self.symbol].pending.orders; 
                    ctPend=len(adjustAsk);
                    
                    while(ctPend>0):
                        ctPend-=1; 
                        pendingUnit=adjustAsk[ctPend];
                        if(pendingUnit.shares<0):
                            discharge = order.cancel(order_id=pendingUnit.order_id);
                            pass;
                            
                elif(self.hopHZ>self.qtyZero):
                    boxA=min(account[self.symbol].position.shares, self.ID_R4[-1][12]);
                    print(boxA);
                    #demitQT = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=boxA, user_key=113, allow_multiple_pending=40);
                    
                    if(account[self.symbol].position.shares>self.ID_R4[-1][12]):
                        boxB=account[self.symbol].position.shares-self.ID_R4[-1][12];
                        print(boxB);
                        #limitQT = order.algo_sell(self.symbol, algorithm='09528d6b-7d4f-4cc2-8611-8bbecb33785c', intent='decrease', order_quantity=boxB, price=self.supply[-1][self.supplyGate], user_key=112, allow_multiple_pending=40);
                        # self.safetySymbol=640
                        pass;
                    pass;
                else:
        ...
        if(self.allocator<0 or self.safetySymbol==750):
            # SWITCHES THE ORDER BOOK SYSTEM TO 750.
            self.safetySymbol=750;
            ...
            if(self.safetySymbol!=555):
                # SWITCHES THE ORDER BOOK SYSTEM TO 640;
                self.safetySymbol=640;
                pass;
            else:
                return 0;

        # USING THE QUARTILE RANGES and 
        def procPL_3(sprPam, qty):
            self.venue+=1;
            if(qty<=account[self.symbol].position.shares):
                if(self.venue%2==1 and qty>0):
                    newLevel = order.algo_sell(self.symbol, algorithm='09528d6b-7d4f-4cc2-8611-8bbecb33785c', intent='decrease', order_quantity=qty, price=self.supply[-1][sprPam]+self.gates[-1][sprPam], user_key=112, allow_multiple_pending=40);
                    self.sizeAgg-=qty;
                    pass;
                elif(self.venue%2==0 and qty>0):
                    newLevel = order.algo_sell(self.symbol, algorithm='f8328375-d95c-4c10-aa7e-0c7fecf13336', intent='decrease', order_quantity=qty, price=self.supply[-1][sprPam]+self.gates[-1][sprPam], user_key=112, allow_multiple_pending=40);
                    self.sizeAgg-=qty;
                    pass;
            else:
                self.sizeAgg=0;
        ...
        def procPL_1():
            ...
            elif(account[self.symbol].unrealized_pl.entry_pl>self.unrlRoll): self.procPL2 = procPL_21();pass;
            elif(account[self.symbol].unrealized_pl.entry_pl<-self.unrlRoll): self.procPL21 = procPL_2();pass;
            return 1;

        if(event.timer_id=="Orderbook"):
            if(self.procPL21==4 or self.procPL2==3):
                chkPxA=0;
                sizeUP=0;
                ...
                if(account[self.symbol].position.shares>0):
                    if(self.procPL21==4): procPL_3(self.procPL21, self.sizeAgg); return;
                    elif(self.procPL2==3): procPL_3(self.procPL2, self.sizeAgg); return;
                    pass;
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.ID_R3[-1][9]==1 and self.postUnit>0):
            # [self.limitBP] 
            # ID_RB1 ALLOCATORS.
            if((account[self.symbol].position.capital_long+account[self.symbol].pending.capital_long)>self.limitBP):
                pass;
            elif(self.ID_RB1[-1][6]==1):
        ...
        # CHECKS FOR THE ALLOCATION RATE CONDITIONS AND DEMITS RISK ACCORDINGLY.
        if(self.chkHtz<0):
            if(account[self.symbol].position.shares>0 and self.qCtr>self.qtyZero):
                adjustRiskB=int(self.ID_R4[-1][13]/self.qtyZero);
                
                if((-account[self.symbol].pending.shares_short+self.postUnit)<account[self.symbol].position.shares):
                    offerInventory = order.algo_sell(self.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='decrease', order_quantity=self.postUnit, price=self.supply[-1][self.supplyGate]+self.gates[-1][4], user_key=117, allow_multiple_pending=40);
                    pass;
                    
                # [self.limitBP] 
                # - puts a GATE UP BEFORE PLACING ANOTHER LIMIT ORDER.
                # [ demitRiskMKT] ] reduce risk by one Unit.
                # - cxl pending LMT BIDS.                
                if((account[self.symbol].position.capital_long+account[self.symbol].pending.capital_long)>self.limitBP):
                    demitRiskMKT = order.algo_sell(self.symbol, algorithm='8fa5330d-b32d-4083-86eb-d9a61a637b8b', intent='decrease', order_quantity=self.postUnit, user_key=116, allow_multiple_pending=40);
                    adjustBids=account[self.symbol].pending.orders; 
                    pndBUY=len(adjustBids);
                    
                    while(pndBUY>0):
                        pndBUY-=1;
                        pendingBID=adjustBids[pndBUY];
                        if(pendingBID.shares>0):
                            killOrder = order.cancel(order_id=pendingBID.order_id);
                            pass;
                        self.orderECN=1;
                    pass;
                    
                elif(account[self.symbol].position.mtm_price<md.L1.last):
                    if((-account[self.symbol].pending.shares_short+self.postUnit)<account[self.symbol].position.shares):
                        demitRisk = order.algo_sell(self.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='decrease', order_quantity=self.postUnit, price=self.supply[-1][self.supplyGate], user_key=118, allow_multiple_pending=40);
                        postLMT = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=self.postUnit, price=self.demand[-1][self.demandGate]-self.gates[-1][4], user_key=212, allow_multiple_pending=40);
                        pass;
                    pass;
            else:
                self.chkHtz=0;
        ...
        # self.ID_R3[-1][9] sells a unit of risk upon discovery of a [0] indicator.
        # puts a GATE UP PRIOR TO ADDING A UNIT OF RISK.
        ...
        if(event.timestamp>self.liquid_time and event.timestamp<self.illiquid_time and self.allocator>1 and self.ID_R3[-1][9]==1):
            ...
            # [self.limitBP] 
            # - puts a GATE UP BEFORE PLACING ANOTHER LIMIT ORDER.
            # - checks the designated risk for the symbol prior to adding a unit of risk.
            if((account[self.symbol].position.capital_long+account[self.symbol].pending.capital_long)>self.limitBP):         
                adjustBids=account[self.symbol].pending.orders; 
                pndBUY=len(adjustBids);
                while(pndBUY>0):
                    ...
                    if(len(idxBar.high)>1 and len(idxBar.low)>1 and len(idxBar.open)>1 and len(idxBar.close)>1 and len(idxBar.bvwap)>1):
                        ...
                        if(len(self.dataBook[0])>1):
                            ...
                            if(dmp>self.cycle and self.quvantity>0):
                                # Alerts the system that the market has moved away from the previous BIDS.
                                if(len(account[self.symbol].pending.orders)>5):
                                    self.orderECN=1;
                                    pass;

                                # [ order_quantity_max ] is set using the [ limitBP ]
                                self.order_quantity_max=int((account[self.symbol].position.capital_long+account[self.symbol].pending.capital_long-self.limitBP)/self._prev_close);
                                
                                if(self.order_quantity_max<1):
                                   return;
                                   # riskMKT_0 = order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='increase', order_quantity=1, user_key=111, allow_multiple_pending=40);
                                   # return;
                                    
                                # order TYPE 1.    
                                # [ riskMKT_1 ] Places a market order to obtain a unit of risk immediately.
                                # [ riskLMT_1 ] Places alimit order to capture the volatility spread.                                
                                if(self.orderECN==1):
                                    self.orderECN=0;
                                    bidQty = int(self.quvantity)+1;

                                    # [riskMKT_1]
                                    riskMKT_1 = order.algo_buy(self.symbol, algorithm='9e6b50d6-dee3-4314-a43b-49e9ae8088b6', intent='increase', order_quantity=min(self.quvantity, self.order_quantity_max), user_key=111, allow_multiple_pending=40);
                                    self.qCtr=0;
                                    # [riskMKT_1]
                                    riskLMT_1 = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=min(self.quvantity, self.order_quantity_max), price=self.demand[-1][self.demandGate]+self.gates[-1][2], user_key=111, allow_multiple_pending=40);
                                    return riskMKT_1, riskLMT_1;
                                else:
                                    # [riskLMT_2]
                                    riskLMT_2 = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='increase', order_quantity=min(self.quvantity, self.order_quantity_max), price=self.demand[-1][self.demandGate]+self.gates[-1][2], user_key=111, allow_multiple_pending=40);
                                    return riskLMT_2;
                            else: return 0;
                        else: return 0;
                    else: return 0;
    ...
    def on_cancel(self, event, md, order, service, account):
        if(event.user_tag==112 and self.safetySymbol!=750 and self.qtyStat!=555):
        ...
        elif(self.qtyStat==555):
            self.hopHZ+=abs(event.shares);
            return self.hopHZ;
        return self.adjustmentOne, self.hopHZ;
    def on_fill(self, event, md, order, service, account):
        
        service.write_file(self.filetestB,'{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}'.format(self.itr, self.symbol, service.time_to_string(service.system_time), service.time_to_string(service.system_time), md.L1.bid,  md.L1.ask,  md.L1.last,self.midPrice, self.pxChg,  self.gates[-1][0], self.gates[-1][1], self.gates[-1][2], self.gates[-1][3], self.gates[-1][4], self.gates[-1][5], self.gates[-1][6], self.gates[-1][7], self.gates[-1][8], self.gates[-1][9], self.gates[-1][10], self.demand[-1][1], self.demand[-1][2], self.demand[-1][3], self.demand[-1][4], self.demand[-1][6], self.supply[-1][1], self.supply[-1][2], self.supply[-1][3], self.supply[-1][4], self.supply[-1][6], self.ID_R3[-1][8], self.ID_R3[-1][9], self.chkMin, self.chkMinPt, self.chkMax, self.chkMaxPt, event.shares, event.price, event.user_tag, self.safetySymbol, self.qtyStat, self.chkHtz, self.allocator, self.sizeAgg, account.buying_power, account[self.symbol].position.shares, account[self.symbol].position.mtm_price, account[self.symbol].unrealized_pl.entry_pl, account[self.symbol].realized_pl.entry_pl, self.qCtr, self.qtyZero, self.quvantity, self.ID_R4[-1][10],self.ID_R4[-1][11], self.ID_R4[-1][12], self.ID_R4[-1][13], self.ID_R4[-1][14], self.ID_R4[-1][15], self.ID_R4[-1][16],  account[self.symbol].pending.count_long, account[self.symbol].pending.shares_long,  account[self.symbol].pending.capital_long,  account[self.symbol].pending.count_short, account[self.symbol].pending.shares_short, account[self.symbol].pending.capital_short));
