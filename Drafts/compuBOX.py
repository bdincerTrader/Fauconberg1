# Editor
# compuBOX   Baris Dincer   6/19/2023 6:26:26 PM   PROD
...
class Gr8Script20528b25a00d489c8b7d7b3e8462f28a(Strategy):
    __script_name__ = 'compuBOX'
    ...
    def on_start(self, md, order, service, account):
        ...
        service.add_time_trigger(md.market_open_time + service.time_interval(minutes=5), repeat_interval=service.time_interval(seconds=30), timer_id="Allocate")
        service.add_time_trigger(md.market_close_time - service.time_interval(minutes=20), timer_id = "eod_A")
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        #if(account[self.symbol].unrealized_pl.entry_pl+account[self.symbol].realized_pl.entry_pl_short > 2000):
        #    return;
        ...
        for idx in range(0, rangeM, 1):
            ...
            if(self.dataBookMS[self.itr][214]==self.dataBookMS[self.itr][213]):
                self.dataBookMS[self.itr][280]=1;
            if(self.dataBookMS[self.itr][236]==self.dataBookMS[self.itr][235]):
                self.dataBookMS[self.itr][281]=1;
                
            self.dataBookMS[self.itr][282] = self.dataBookMS[self.itr][279] + self.dataBookMS[self.itr][280] + self.dataBookMS[self.itr][281];

            if(self.idxLoop>10 and idx==rangeM-2):
                
                grA=self.dataBookMS[self.itr][279];
                grB=self.dataBookMS[self.itr][282];
                grS=self.dataBookMS[self.itr][257]/5;
                grT=self.dataBookMS[self.itr][257]/2;
                
                if(self.dataBookMS[self.itr][257]<self.dataBookMS[self.itr-1][257]): grB+1; pass;

                if(grA==0):
                    print(grA, grB, grS, md.L1.bid, md.L1.ask)
                    ...
                    if(account[self.symbol].position.shares!=0):
                        pxC=md.L1.bid-grT;
                        print('BUY :: COVER [md.L1.bid-grS]\t', pxC)
                        cover = order.algo_buy(self.symbol, algorithm='4ad20d14-aacb-4850-9efe-cfa92d154304', intent='exit', price=pxC, user_key=151, allow_multiple_pending=True);
                        break;
                    else:
                        break;
                    
                elif(grB>=8 and grB<25):
                    print(grA, grB, grS, md.L1.bid, md.L1.ask)
                    
                    if(grB%2==0):
                        pxA=md.L1.bid+grT;
                        print('SELL: 0 [md.L1.bid+grT]\t', pxA)
                        shrtA = order.algo_sell(self.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='increase', order_quantity=grB, price=pxA, user_key=9, allow_multiple_pending=True)
                    else:
                        pxB=md.L1.ask-grT;
                        print('SELL: 1 [md.L1.ask-grS]\t', pxB)
                        shrtB = order.algo_sell(self.symbol, algorithm='c982d0cd-9be5-4a35-b89f-1f66b2495ec4', intent='increase', order_quantity=grB, price=pxB, user_key=9, allow_multiple_pending=True)
            ...
            elif(self.idxLoop>10000000000000):
