# Editor
# Fauconberg   Baris Dincer   6/12/2023 1:07:11 PM   PROD
...
class Gr8Scriptcffda23257ae4d54a108e8990036b304(Strategy):
    ...
    bpRisk=25000; cycl=5; askRTE=1.0026; bpMaxLoss=-bpRisk*0.20; plUnrlRTE=bpRisk*0.0010; plUpperBand=bpRisk*.01; plLowerBand=-bpRisk*.01; plBandMult=bpRisk*.01; 
    ...
    def on_start(self, md, order, service, account):
        ...
        service.add_time_trigger(md.market_open_time + service.time_interval(minutes=10), repeat_interval=service.time_interval(seconds=5), timer_id="Allocate")
    ...
    def on_timer(self, event, md, order, service, account):
        ...
        def procPL_3(sprPam):
            chkPxA=0; Bpx=md.L1.bid; Apx=md.L1.ask;
            if(sprPam==2): qbA=Bpx+((Apx-Bpx)/sprPam); reRack=max(md.L1.bid, qbA);pass;
            elif(sprPam==4): qbA=Apx-((Apx-Bpx)/sprPam); reRack=max(md.L1.last, qbA);pass;
            reRack=int(reRack*10000 + 0.001) / 10000.0;
            dupeChk=int((random.randint(1, 50)/10000) * 10000 + 0.001) / 10000.0;
        ...
        def procPL_1():
            ...
            if(account[self.symbol].position.mtm_price*1.0005<md.L1.bid): self.procPL2 = procPL_2();pass;
            elif(account[self.symbol].position.mtm_price*1.0005<md.L1.last): self.procPL2 = procPL_2();pass;
            elif(account[self.symbol].position.mtm_price>md.L1.bid*1.0005): self.procPL21 = procPL_21();pass;
            elif(account[self.symbol].unrealized_pl.entry_pl>self.__class__.plUnrlRTE): self.procPL2 = procPL_2();pass;
            elif(account[self.symbol].unrealized_pl.entry_pl<-self.__class__.plUnrlRTE): self.procPL21 = procPL_21();pass;
            return 1;
        ...
        if(self.procPL21==2): procPL_3(2);pass;
